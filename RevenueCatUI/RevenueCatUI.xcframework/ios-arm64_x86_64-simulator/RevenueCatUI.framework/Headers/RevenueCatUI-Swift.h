...
///
/// \param key The variable key (without the <code>custom.</code> prefix). Invalid keys are ignored.
///
...
- (void)paywallViewControllerDidOpenWebCheckout:(RCPaywallViewController * _Nonnull)controller;
/// Notifies that a <code>PaywallViewController</code> successfully opened a URL, either from a button with a URL
/// destination or from a link inside a text component.
/// Not called for web checkout URLs. Use <code>paywallViewControllerDidOpenWebCheckout(_:)</code> for those.
- (void)paywallViewController:(RCPaywallViewController * _Nonnull)controller didOpenURL:(NSURL * _Nonnull)url;
...
@class UIWindow;
@class UIWindowScene;
@interface RCPurchases (SWIFT_EXTENSION(RevenueCatUI))
/// Attempts to present a paywall from a Preview Paywall deep link.
/// This method parses the provided <code>URL</code> and attempts to extract information that correlates
/// to a known offering and published paywall. If successful, this method returns <code>true</code> and
/// attempts to present that paywall for previewing.
/// The <code>viewController</code> parameter may be <code>nil</code>. If omitted, the SDK will attempt to locate a suitable
...
/// Use this method when handling a request for your app to open a <code>URL</code>. For example:
/// \code
...
/// }
///
/// \endcode\param url The <code>URL</code> received by your application
///
/// \param viewController The view controller to present the paywall from. In unspecified (the default),
/// a suitable view controller will be chosen automatically.
///
///
/// returns:
/// <code>true</code> if the URL is a valid rc-paywall-preview URL and handling has begun; <code>false</code> otherwise.
- (BOOL)presentPaywallFrom:(NSURL * _Nonnull)url viewController:(UIViewController * _Nullable)viewController SWIFT_WARN_UNUSED_RESULT SWIFT_AVAILABILITY(macos,introduced=12.0) SWIFT_AVAILABILITY(ios,introduced=15.0);
/// Attempts to present a paywall from a Preview Paywall deep link.
/// This method parses the provided <code>URL</code> and attempts to extract information that correlates
/// to a known offering and published paywall. If successful, this method returns <code>true</code> and
/// attempts to present that paywall for previewing.
/// The <code>window</code> parameter is optional. If omitted, the SDK will attempt to locate a suitable
...
/// \endcode\param url The <code>URL</code> received by your application
///
/// \param window The window to present the paywall from. In unspecified,
/// a suitable window will be chosen automatically.
///
///
/// returns:
/// <code>true</code> if the URL is a valid rc-paywall-preview URL and handling has begun; <code>false</code> otherwise.
- (BOOL)presentPaywallFrom:(NSURL * _Nonnull)url window:(UIWindow * _Nullable)window SWIFT_WARN_UNUSED_RESULT SWIFT_AVAILABILITY(macos,introduced=12.0) SWIFT_AVAILABILITY(ios,introduced=15.0);
/// Attempts to present a paywall from a Preview Paywall deep link.
/// This method parses the provided <code>URL</code> and attempts to extract information that correlates
/// to a known offering and published paywall. If successful, this method returns <code>true</code> and
/// attempts to present that paywall for previewing.
...
/// returns:
/// <code>true</code> if the URL is a valid rc-paywall-preview URL and handling has begun; <code>false</code> otherwise.
- (BOOL)presentPaywallFrom:(NSURL * _Nonnull)url scene:(UIWindowScene * _Nullable)scene SWIFT_WARN_UNUSED_RESULT SWIFT_AVAILABILITY(macos,introduced=12.0) SWIFT_AVAILABILITY(ios,introduced=15.0);
/// Attempts to present a paywall from a Preview Paywall deep link.
/// This method parses the provided <code>URL</code> and attempts to extract information that correlates
/// to a known offering and published paywall. If successful, this method returns <code>true</code> and
...
/// returns:
/// <code>true</code> if the URL is a valid rc-paywall-preview URL and handling has begun; <code>false</code> otherwise.
- (BOOL)presentPaywallFrom:(NSURL * _Nonnull)url SWIFT_WARN_UNUSED_RESULT SWIFT_AVAILABILITY(macos,introduced=12.0) SWIFT_AVAILABILITY(ios,introduced=15.0);
@end
